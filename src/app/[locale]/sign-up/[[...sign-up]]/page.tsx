...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signUp.status === "complete") {
        ...
        if (sendErr) {
          setError(sendErr.message || t("errorDefault"));
          setLoading(false);
          return;
        }
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (verifyErr) {
        setError(verifyErr.message || t("errorDefault"));
        setLoading(false);
        return;
      }
      ...
      <Card className="w-full max-w-sm border border-zinc-200/60 p-8 dark:border-zinc-800">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-4 text-xl font-semibold text-zinc-900 dark:text-zinc-100">
            {needsVerification ? "Check your email" : t("signUpTitle")}
          </h1>
        ...
        <p className="mt-8 text-center text-sm text-zinc-500 dark:text-zinc-400">
          {t("hasAccount")}{" "}
          <a href="/sign-in" className="font-medium text-zinc-900 underline underline-offset-2 hover:text-zinc-600 dark:text-zinc-100 dark:hover:text-zinc-400">
            {t("signInLink")}
          </a>
        </p>
      </Card>
